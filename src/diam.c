...
//------------------------------------------
// MODIFIED BY Benjamin Ravalijaona
//------------------------------------------
...
int tree_max_dist(int *t, int n){
  ...
  while (1) {
    ...
    subtree_max_dist[t[v]] = max(subtree_max_dist[t[v]],
				 max1_subsubtree_height[t[v]]
				 + max2_subsubtree_height[t[v]]);
...
//--------------------------------------------------
// Recuperation des 2 derniers niveaux graphes
//--------------------------------------------------
int *get_lastBFS(int *t, int *list, int v)
{
...
int main(int argc, char **argv){
  ...
  int tlb, diam, rtub, dslb, tub,  hdtub, nb_max, prec_option, fringe;
  int deg_begin=0;
  float precision;
  ...
  tlb = 0; diam = 0; prec_option = 0; dslb = 0; tub = 0; rtub = 0, hdtub = 0; fringe = 0;
  ...
  for (i = 1; i < argc; i++) {
    ...
    //----------------------------------------
    // Ajout de l'option fringe
    //----------------------------------------
    else if (strcmp(argv[i], "-fringe") == 0) {
	fringe = 1;
  ...
  if ((tlb + diam + prec_option + rtub + dslb + tub + hdtub + fringe) != 1) {
    usage(argv[0]);
  }
  ...
  ** and highest degree tree upper bound
  ** for the diameter
  */
  else if (diam) {
    ...
    printf("#1:i=iteration_number"
	   "#2:i-th_lower_node"
	   "#3:i-th_upper_node" 
	   "#4:degree_of_i-th_lower_node" 
	   "#5:degree_of_i-th_upper_node" 
	   "#6:i-th_lower_bound " 
	   "#7:i-th_upper_bound" 
	   "#8:i-th_best_lower_bound" 
	   "#9:i-th_best_upper_bound\n");
  ...
  if(dslb) {
    ...
    printf("#1:iteration_number" 
	   "#2:i-th_node"
	   "#3:degree_of_ith_node" 
	   "#4:i-th_lower_bound\n");
